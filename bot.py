...
# código sempre diferente (random)
...
"""
Tenta DM. Se falhar, manda aviso no canal fallback (ex: #ligar-conta-e-verificar).
"""
try:
...
# =========================
# DB INIT (IBAN + SUPORTE + CAMPANHAS + VERIFICAÇÃO)
# =========================
def init_db():
conn = db_conn()
cur = conn.cursor()
...
status TEXT NOT NULL DEFAULT 'pending', -- pending/approved/rejected/frozen
...
# Discord só deixa o link clicável se tiver https:// (e não tiver caracteres estranhos)
...
super().__init__(timeout=None)
self.campaign_slug = str(campaign_slug)
...
conn = db_conn()
row = get_campaign_by_slug(conn, self.campaign_slug)
if not row:
conn.close()
return await _safe_ephemeral(interaction, "❌ Campanha não encontrada.")
...
guild.get_role(VERIFICADO_ROLE_ID): discord.PermissionOverwrite(read_messages=True, send_messages=True),
}
admin_member = guild.get_member(ADMIN_USER_ID)
if admin_member:
overwrites[admin_member] = discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_messages=True)
...
if not subs or not active_campaigns:
return
...
# Guarda no DB (persistente) — e o código é sempre random
...
code = generate_verification_code() # ✅ agora muda sempre
...
# =========================
# APROVAR / REJEITAR (VERIFICAÇÃO) + NOTIFICAÇÃO + DB
# =========================
...
# Notifica user (DM; se falhar, fallback no canal ligar-conta)
await notify_user(
member,
...
f"• Vai ao canal <#{LIGAR_CONTA_E_VERIFICAR_CHANNEL_ID}> e usa **!ibanpanel** (ou pede ao admin para fixar o painel).\n",
fallback_channel_id=LIGAR_CONTA_E_VERIFICAR_CHANNEL_ID
)

# desativar botões na mensagem
for child in self.children:
child.disabled = True
...
# desativar botões e atualizar msg
for child in self.children:
child.disabled = True
...
# limpa IBAN e pedido de verificação (para recomeçar do zero)
delete_iban(member.id)
delete_verification_request(member.id)

await ctx.send(f"♻️ {member.mention} foi **desverificado** e os dados (IBAN + verificação) foram limpos.\n"
"✅ Ele pode repetir o processo no painel.")

# Nota: as categorias das campanhas ficam “invisíveis” automaticamente ao tirar o cargo Verificado,
# porque as permissões estão por cargo (read_messages = False para everyone).
...
# =========================
# READY: reanexar botões nas msgs antigas (verificação + submit panel)
# =========================
...
# se tiver msg antiga, tenta editar e reanexar view (botões voltam a funcionar após restart)
try:
...
# se não conseguir buscar, reposta uma nova msg
try:
...
# Views persistentes base (painéis)
...
# Reanexar botões em mensagens já existentes (corrige “botões não funcionam” após restart)
try:
